...
var CLI struct {
	...
	YouTube struct {
		...
		FeedType      string `help:"Feed type: channel or playlist" default:"channel" enum:"channel,playlist"`
		FeedID        string `help:"YouTube channel ID or playlist ID" required:"true"`
		EnableDeArrow bool   `help:"Enable DeArrow title enrichment" default:"true"`
	} `cmd:"youtube" help:"Generate RSS feed from YouTube channel/playlist with DeArrow enrichment."`
	...
	Preview struct {
		...
		YouTube struct {
			...
			Limit         int    `help:"Maximum number of items" default:"10"`
			Index         int    `help:"Output XML for specific item index (0-based) to stdout" default:"-1"`
		} `cmd:"youtube" help:"Preview YouTube feed items."`
...
func main() {
	...
	case "youtube":
		...
		// Create provider config from CLI arguments
		providerConfig := &youtube.Config{
			FeedType:      CLI.YouTube.FeedType,
			FeedID:        CLI.YouTube.FeedID,
			EnableDeArrow: CLI.YouTube.EnableDeArrow,
		}
		...
		if err := provider.GenerateFeed(CLI.YouTube.Outfile, false); err != nil {
			slog.Error("Failed to generate YouTube feed", "error", err)
			os.Exit(1)
		}

	case "preview youtube":
		slog.Debug("Previewing YouTube feed...")

		// Create provider config from CLI arguments
		providerConfig := &youtube.Config{
			FeedType:      CLI.Preview.YouTube.FeedType,
			FeedID:        CLI.Preview.YouTube.FeedID,
			EnableDeArrow: CLI.Preview.YouTube.EnableDeArrow,
		}
		...
		// Fetch items
		items, err := provider.FetchItems(CLI.Preview.YouTube.Limit)
		if err != nil {
			slog.Error("Failed to fetch YouTube items", "error", err)
			os.Exit(1)
		}

		// Define feed configuration (same as used in GenerateFeed)
		var feedTitle, feedLink string
		if CLI.Preview.YouTube.FeedType == "channel" {
			feedTitle = "YouTube Channel Feed"
			feedLink = fmt.Sprintf("https://www.youtube.com/channel/%s", CLI.Preview.YouTube.FeedID)
		} else {
			feedTitle = "YouTube Playlist Feed"
			feedLink = fmt.Sprintf("https://www.youtube.com/playlist?list=%s", CLI.Preview.YouTube.FeedID)
		}

		if CLI.Preview.YouTube.EnableDeArrow {
		...
		// If index is specified, output XML directly to stdout
		if CLI.Preview.YouTube.Index >= 0 {
			if CLI.Preview.YouTube.Index >= len(items) {
				slog.Error("Index out of range", "index", CLI.Preview.YouTube.Index, "total", len(items))
				os.Exit(1)
			}
			xml := preview.FormatXMLItem(items[CLI.Preview.YouTube.Index], "youtube-atom", feedConfig)
			fmt.Println(xml)
			return
		}

		// Run preview TUI with template
		if err := preview.Run(items, "YouTube", "youtube-atom", feedConfig); err != nil {
			slog.Error("Preview failed", "error", err)
			os.Exit(1)
		}
