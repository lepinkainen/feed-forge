...
// GenerateTemplateFeed generates a feed using template-based generation
func (p *HackerNewsProvider) GenerateTemplateFeed(outfile string, reauth bool) error {
	// Clean up expired entries using base provider
	if err := p.CleanupExpired(); err != nil {
		// Non-fatal error, just warn
	}
	...
	// Generate and save the feed using template
	rss, err := generateTemplateFeed(contentDB.DB(), ogDB, allItems, p.MinPoints, p.CategoryMapper)
	if err != nil {
		return err
	}
	if err := os.WriteFile(outfile, []byte(rss), 0644); err != nil {
	...
	slog.Info("Template-based Hacker News feed generated", "items", len(allItems), "output", outfile)
	return nil
}
